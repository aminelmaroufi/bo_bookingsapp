...
import { authActions, AuthState } from "src/types";
...
describe("test auth reducer", () => {
  ...
  it("should handle the 'LOGOUT_SUCCESS' action", () => {
    ...
    const logoutSuccessPayload: authActions = {
      type: ActionTypes.LOGOUT_SUCCESS,
    };

    expect(auth(undefined, logoutSuccessPayload)).toEqual(expectedState);
  });

  it("should handle the 'RESET_PASSWORD_SUCCESS' action", () => {
    const expectedState = { ...defaultState, reset_pwd_success: true };
    const resetPwdSuccessPayload: authActions = {
    ...
    expect(auth(undefined, resetPwdSuccessPayload)).toEqual(expectedState);
  });
});
