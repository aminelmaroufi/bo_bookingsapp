...
const UpdateRoomDialog = (props) => {
  const [room, setRoom] = useState(props.room);
  const [selectedField, setSelectedField] = useState("");
  const [titleIsValid, setTitleIsValid] = useState(true);
  const [titleValueIsValid, setTitleValueIsValid] = useState(true);
  const [roomPictureIsValid, setRoomPictureIsValid] = useState(true);
  const [priceIsValid, setPriceIsValid] = useState(true);
  const [advantageIsValid, setAdvantageIsValid] = useState(true);
  const [advantageValueIsValid, setAdvantageValueIsValid] = useState(true);
          ...
          {room.room_picture && (
            <Box mt={2} style={{ marginBottom: 15 }}>
              ...
              <div style={{ flexDirection: "row" }}>
                <img
                  src={
                    typeof room.room_picture === "string"
                      ? `${baseURL}/files/${room.room_picture}/view`
                      : URL.createObjectURL(room.room_picture)
                  }
                  alt={"main_picture"}
                  height="100px"
                />
